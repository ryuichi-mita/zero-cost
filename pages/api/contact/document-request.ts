...
function replaceVariables(template: string, formData: FormData): string {
  return template
    ...
    .replace(/\{message\}/g, formData.message || '資料請求');
}
...
export default async function handler(
  ...
  try {
    ...
    const userTemplate = getEmailTemplate('mail-user');
    ...
    const userSubject = replaceVariables(userTemplate.subject, formData);
    const userBody = replaceVariables(userTemplate.body, formData);
